...
- `DESIGN.md` contains `CDP:UNCONFIGURED` or ss1 template filler (`Describe the product's visual register`)
...
## READY
O harness esta configurado. [Respond to user message or ask first-surface question.]
```
...
- **Template DESIGN.md left in place:** ss1 still says "Describe...". -> Phase 5 is mandatory.
...
Also triggers on `oi`, `ola`, or any first message when unbound.

Expected: full auto-setup runs silently, then responds with HARNESS AUTO-SETUP report + "O que voce quer desenhar primeiro?"
